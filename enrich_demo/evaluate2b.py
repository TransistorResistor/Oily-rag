#!/usr/bin/env python3
"""
evaluate.py - score proposals against gold2b.json (corpus 2b).
...
def load(props_path=None):
    # Optional CLI arg selects the proposals file (default proposals2b.json), so a
    # per-DB proposals_<stem>.json can be scored without renaming. A bare stem or
    # relative name is resolved against HERE.
    gold = json.load(open(os.path.join(HERE, "gold2b.json"), encoding="utf-8"))
    pp = props_path or "proposals2b.json"
...
def gold_proposals(gold):
    """Flatten gold2b.json into expected surfaced proposals + expected zero docs."""
    exp = []
    zero_docs = set()
    parks = []
    for g in gold:
...
def main():
    ...
    print("=" * 68)
    print("REVERSE-ENRICHMENT EVAL  (proposals2b.json vs gold2b.json)")
    print("=" * 68)
