#!/usr/bin/env python3
"""
evaluate.py - score the final proposals.json against gold.json.
...
def load(props_path=None):
    # Optional CLI arg selects the proposals file (default proposals.json), so a
    # per-DB proposals_<stem>.json can be scored without renaming.
    gold = json.load(open(os.path.join(HERE, "gold.json"), encoding="utf-8"))
    pp = props_path or "proposals.json"
...
def gold_proposals(gold):
    """Flatten gold.json into expected surfaced proposals + expected zero docs."""
    exp = []
    zero_docs = set()
    parks = []
    for g in gold:
...
def main():
    ...
    print("=" * 68)
    print("REVERSE-ENRICHMENT EVAL  (proposals.json vs gold.json)")
    print("=" * 68)
